...
export function ConfigPanel({ node, onClose, onSave }: ConfigPanelProps) {
  const componentInfo = node
    ? getComponentByType(node.data.componentType)
    : null;
  const {
    handleSubmit,
    control,
  ...
  const watchedOptionalFields = useWatch({
    control,
    name:
      componentInfo?.params
        .filter((p) => p.optional)
        .map((p) => `${p.name}__active`) || [],
  });
    ...
    if (node && componentInfo) {
      ...
      componentInfo.params.forEach((param) => {
        ...
        if (param.optional) {
          ...
          defaultValues[param.name] = isActive
            ? paramValue
            : param.defaultValue;
        } else {
  ...
  const renderField = (param: Param, index: number) => {
    ...
    const isOptionalActive = param.optional
      ? watchedOptionalFields[index]
      : true;
      ...
      <div key={param.name} className="space-y-2">
        ...
        <div className={cn("grid gap-2", !isOptionalActive && "hidden")}>
          ...
          <Controller
            ...
            rules={{
              required:
                !param.optional&& isOptionalActive
  ...
  const optionalParamStartIndex =
    componentInfo?.params.findIndex((p) => p.optional) ?? -1;
          ...
          >
            <ScrollArea className="flex-grow pr-6 -mr-6">
              <div className="space-y-4 py-4">
                {componentInfo.params.map((param, index) =>
                  renderField(
                    param,
                    param.optional ? index - optionalParamStartIndex : -1
                  )
                )}
              </div>
            </ScrollArea>
